...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        valid_square = False
        val = None
        while not valid_square:     # while valid_square == False
...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.available_moves()) # choose one randomly
        else: 
            # get the square based of the minimax algo
    ...
    def minimax(self, state, player):
        ...
        for possible_move in state.available_moves():
            ...
            # step 4: update the dictionary if necesary
            if player == max_player:
